...
func (h *FileHook) Setup() error {
	...
	nono := time.Now().UnixNano()
	filepaths := strings.Split(h.FileName, ".")
	if len(filepaths) > 1 {
		h.FileName = strings.Join(filepaths[0:len(filepaths)-1], "") + "-" + strconv.FormatInt(nono, 10) + "." + filepaths[len(filepaths)-1]
	} else {
		h.FileName = h.FileName + "-" + strconv.FormatInt(nono, 10)
	}
	rotateTime := v.GetString(keyFileRotateTime)

	if _, err = os.Stat(h.FilePath); err != nil {
		if os.IsNotExist(err) {
			if err := os.MkdirAll(h.FilePath, 0666); err != nil {
				return err
			}
		}
	}
...
var _InitFileHook = func() interface{} {
	...
	cli.String(keyFileName, "glog.log", "logger.file.name")
